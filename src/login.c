...
{
    int ok;
    const char* creds[2] = {username, ""};  // Username and blank password
    struct pam_conv conv = {login_conv, creds};
    struct pam_handle* handle;

    ok = pam_start(config.service_name, username, &conv, &handle);
    ...
    ok = pam_authenticate(handle, 0);  // Directly call pam_authenticate without wrapper
