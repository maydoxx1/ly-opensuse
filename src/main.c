...
void *fingerprint_auth_thread(void *arg)
{
    struct term_buf *buf = (struct term_buf *)arg;
    if_pam_successful = fingerprint_auth(buf, login_username);  // Use global login_username
    return NULL;
}
...
int main(int argc, char** argv)
{
    ...
    // Start fingerprint authentication thread
    pthread_t fingerprint_thread;
    pthread_create(&fingerprint_thread, NULL, fingerprint_auth_thread, &buf);
    ...
    while (run)
    {
        ...
        if (if_pam_successful) {
            // Call post_auth function to start the desktop environment
            post_auth(&desktop, &login, &buf);
            buf.info_line = lang.logout;
            //run = false;
            load(&desktop, &login);
            system("tput cnorm");
